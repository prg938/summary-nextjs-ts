
import styles from '@/styles/Input.module.scss'
import React, {FC, useEffect} from "react"

interface InputType {
  debounce: (...args: any[]) => any
  delay?: number
}

const Input: FC<InputType> = ({debounce, delay = 250}) => {
  const [text, setText] = React.useState<string>(String())
  
  let timeout: Parameters<typeof clearTimeout>[0] = undefined

  const inputHandler = (event: React.KeyboardEvent<HTMLInputElement>) => {
    const text = event.currentTarget.value
    setText(text)
  }
  useEffect(() => {
    timeout = setTimeout(() => debounce(text), delay)
    return () => {
      clearTimeout(timeout)
    }
  }, [text])

  return <div className={styles.searchWrapper}>
    <input type="text" placeholder="Найти по названию" className={styles.searchInput} onInput={inputHandler} value={text} />
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#5F6368" className={styles.searchSVG}>
      <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path>
    </svg>
  </div>
}

export default Input