...
const GistFileComponent: FunctionComponent<{URL: string}> = ({URL}) => {
  ...
  const opts = {...colorScheme, language: colorScheme?.theme !== ColorSchemes.Text ? defaultLanguage : undefined}
  const isMDExtension = /\.md/i.test(URL)
  if (isMDExtension) opts.language = 'text'
  ...
  return (
    <Highlight {...opts} customStyle={{fontSize: '12px', background: 'transparent'}}>{result}</Highlight>
  )
}
